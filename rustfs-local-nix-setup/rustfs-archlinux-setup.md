# Setting up RustFS on Arch Linux for Local Development

RustFS is an S3-compatible distributed object storage system written in Rust. This
guide walks through getting it running locally on Arch for development work.

## 1. Install system dependencies

Arch uses `pacman`. You'll need a C/C++ toolchain, OpenSSL headers, and a few
...
- `base-devel` group — pulls in `gcc`, `make`, `pkgconf`, and base build tools.
- `llvm` — provides `libclang`, needed by `bindgen`-based crates that generate
  FFI bindings at build time (`clang` alone is the frontend compiler).
- `openssl` — on Arch the package includes headers, so `openssl-sys` links
  against it directly.
- `pkgconf` — Arch's `pkg-config` implementation; part of `base-devel` but
  listed for clarity.
- `cmake` — some transitive native deps use it.
- `protobuf` — provides the `protoc` binary required by `prost-build` (used by
  the `pulsar` crate at build time). Without it, the build fails with
  `Could not find protoc`.

## 2. Install the Rust toolchain

Arch ships a current Rust in the `extra` repo, but for development you generally
want `rustup` so you can pin toolchain versions and add components easily:
...
# Option B: distro package (Arch's is usually very current)
# sudo pacman -S rust
```
...
## Common pitfalls on Arch

- **Linker errors mentioning OpenSSL** → ensure `openssl` is installed (headers
  are bundled in the package on Arch). Alternatively, some Rust projects let you
  use the vendored feature: `cargo build --features vendored` or set
  `OPENSSL_DIR`.
- **`bindgen` / `libclang` errors** → install `llvm` (provides `libclang.so`).
  `clang` alone is not always sufficient.
- **`Could not find protoc`** → install `protobuf` (provides the `protoc`
  binary that `prost-build` invokes via the `pulsar` crate). As a fallback, set
  `export PROTOC=/usr/bin/protoc`.
- **Partial upgrades** → Arch is rolling; always `pacman -Syu` before building.
  Mixing a stale system with freshly built Rust crates can produce confusing
  ABI/link errors.
