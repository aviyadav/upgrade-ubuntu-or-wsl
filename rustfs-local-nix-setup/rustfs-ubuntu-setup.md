# Setting up RustFS on Ubuntu for Local Development

RustFS is an S3-compatible distributed object storage system written in Rust. This
guide walks through getting it running locally on Ubuntu for development work.

## 1. Install system dependencies

Ubuntu uses `apt`. You'll need a C/C++ toolchain, OpenSSL headers, and a few
...
- `build-essential` — pulls in `gcc`, `g++`, `make`, and base build tools.
- `libclang-dev` / `clang` — needed by `bindgen`-based crates that generate FFI
  bindings at build time.
- `libssl-dev` + `pkg-config` — the `openssl-sys` crate links against system
  OpenSSL.
...
Ubuntu ships Rust in its repos, but for development you generally want `rustup`
...
# Option B: distro package (often lags behind latest stable)
...
## Common pitfalls on Ubuntu

- **Linker errors mentioning OpenSSL** → `libssl-dev` or `pkg-config` missing.
  Alternatively, some Rust projects let you use the vendored feature:
  `cargo build --features vendored` or set `OPENSSL_DIR`.
- **`bindgen` / `libclang` errors** → install `libclang-dev`.
...
- **Stale distro Rust** → Ubuntu's `rustc` package is often several versions
  behind. If you hit "unsupported toolchain" errors during build, switch to
  `rustup`.
